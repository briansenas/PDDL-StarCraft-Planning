(define (domain ejer8)
    ...
    (:functions
        ...
        ;; Coste de cada una de las acciones
        (costeConstruir ?x - tipoEdificio)
        (costeEntidad ?x - tipoUnidad)
        (costeNavegar ?x - tipoUnidad)
    )
    (:predicates
    ...
    (:action Navegar
        ...
        :precondition
            (and
                ...
                (esUnidad ?u ?tr)
                (< (+ (coste plan) (costeNavegar ?tr)) (costeMinimo esperado) )
                (unidadeEn ?u ?x)
                (caminoEntre ?x ?y)
            )
        :effect
            (and
                (not (unidadeEn ?u ?x) )
                (unidadeEn ?u ?y)
                (increase (coste plan) (costeNavegar ?tr) )
            )
    )
    ...
    (:action ConstruirEdificio
        ...
        :precondition
            (and
                (esEdificio ?e ?ed)
                (< (+ (coste plan) (costeConstruir ?ed)) (costeMinimo esperado) )
                (esUnidad ?u VCE)
                (unidadeEn ?u ?x)
        ...
        :effect
            (and
                (edificioEn ?e ?x)
                (construido ?e)
                (increase (coste plan) (costeConstruir ?ed) )
    ...
    (:action Reclutar
        ...
        :precondition
            (and
                ...
                (< (+ (coste plan) (costeEntidad ?ut)) (costeMinimo esperado) )
                (edificioEn ?e ?x)
                (exists (?ed -  tipoEdificio)
                     (and
                      (esEdificio ?e ?ed)
                      (generadoEn ?ut ?ed)
                     )
        ...
        :effect
            (and
                (unidadeEn ?u ?x)
                (increase (coste plan) (costeEntidad ?ut) )
                (forall (?tr - tipoRecurso )
                    (when (requiere ?ut ?tr)
    ...
    (:action Recolectar
        ...
        :precondition
            (and
                (< (+ (coste plan) 5) (costeMinimo esperado) )
                (esRecurso ?r ?tr)
